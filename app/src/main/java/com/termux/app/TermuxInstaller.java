...
final class TermuxInstaller {
    ...
    static void setupBootstrapIfNeeded(final Activity activity, final Runnable whenDone) {
        ...
        if (FileUtils.directoryFileExists(TERMUX_PREFIX_DIR_PATH, true)) {
            ...
            } else if (new File(TERMUX_BIN_PREFIX_DIR_PATH, "bash").exists()) {
                whenDone.run();
                return;
            } else {
                Logger.logInfo(LOG_TAG, "The termux prefix directory \"" + TERMUX_PREFIX_DIR_PATH + "\" exists but bash is missing, re-extracting bootstrap.");
            }
    ...
    static void fixPrefixSymlinks(String prefixPath) {
        ...
        if (fixed > 0) {
            Logger.logInfo(LOG_TAG, "Upgrade: fixed " + fixed + " broken symlinks under " + prefixPath);
        }
    }

    private static int fixSymlinksRecursive(File dir, String oldPrefix, String newPrefix) {
        File[] files = dir.listFiles();
        if (files == null) return 0;
        int fixed = 0;
        for (File file : files) {
            if (file.isDirectory()) {
                fixed += fixSymlinksRecursive(file, oldPrefix, newPrefix);
            } else {
                try {
