...
public class VenvExtractor {
    ...
    public static boolean extractVenv(Context context) {
        ...
        Logger.logInfo(LOG_TAG, "Verify: venvDir exists=" + venvDirFile.exists() + " bin exists=" + binDir.exists() + " python exists=" + pythonBin.exists());
        if (binDir.exists()) {
            String[] binContents = binDir.list();
            if (binContents != null && binContents.length > 0) {
                StringBuilder sb = new StringBuilder("bin/ contents (first 10): ");
            ...
            // List what's actually in venvDir
